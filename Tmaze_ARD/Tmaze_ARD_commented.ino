...
servo5.write(pos5); 
pos5=pos5+1;tic2=millis(); //when FOOD_SPEED 20ms has passed, servo moves 1deg and tic2 updated
    }}
    }
    if (pos5 == SHOW_FOOD){
  ...
  if (duration>interval){ //timer criterion
    maze_mode=3; //maze_mode for removing food and braking run-wheel
    if (time_flag2==1){tic=millis();time_flag=1;time_flag2=0;time_flag3=1;}//time_flag2 so tic, time_flag and time_flag3 only updated once at start
  servo4.write(BRAKE_WHEEL);
  //non-blocking for loop to move food away without delays to code since animal could be elsewhere.
       if (time_flag==1){tic2=millis();time_flag=0;} //update only once at start
    if (pos5 > HIDE_FOOD){
    duration2=millis()-tic2;
...
pos5=pos5-1;
    ...
    if (time_flag3==1){tic=millis();time_flag3=0;}//update only once at start
  servo4.write(BRAKE_WHEEL);
  //non-blocking for loop to move food away without delays to code since animal could be elsewhere.
    if (pos5 > HIDE_FOOD){ 
...
pos5=pos5-1;
  ...
  if (photo_value1 < 0.8*INIT_READ1){    //BB1 tripped
