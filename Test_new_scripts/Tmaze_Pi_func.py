...
def append_data(trigger, event, animal=animaltag, time=time.strftime("%H%M%S"), date=time.strftime("%Y%m%d")):
    db_list["Animal"].append(animal)
    db_list["Triggered by"].append(trigger)
    db_list["Event"].append(event)
    db_list["Time"].append(time)
    db_list["Date"].append(time)
    ...
    while True:
        if MODE == 1:
            ...
            if len(ser_string) > 5:
                ...
                append_data("RFID", "Session started")
        ...
        if MODE == 2 and GPIO.input(ard_pi_1): 
            ...
            #appending data to database
            append_data("Beam Break 1", "Weight = " + weight_data)
        ...
        if MODE == 3 and GPIO.input(ard_pi_2) and flag==1: #log a trial start
            ...
            #appending data to database
            append_data("Beam Break 2", "Run cycles = " + cycles_str)
        ...
        if MODE == 3 and GPIO.input(ard_pi_3): #animal going back home
            ...
            #appending data to database
            append_data("Beam Break 3", "Return to homecage")
        ...
        if MODE == 3 and GPIO.input(ard_pi_4) and flag==0: #log a maze event food
            ...
            #appending data to database
            append_data("Beam Break 4", "Food pod")
        ...
        #appending data to database
        append_data("Beam Break 5", "Running wheel")
