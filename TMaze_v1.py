...
GPIO.add_event_detect(
    19, GPIO.RISING, callback=emergency_button
...
def RFID_datacheck(clean_tag):
    ...
    """
    if clean_tag == "137575399500":
        animaltag = "194882"
        print("Animal in: LONG_WHITE. ID: 194882")
        return animaltag

    elif clean_tag == "137506193448":
        animaltag = "194883"
        print("Animal in: Right ear. ID: 194883")
        return animaltag

    elif clean_tag == "2006010337":
        animaltag = "194884"
        print("Animal in: Left ear. ID: 194884")
        return animaltag
...
def RFID_sequential_check(animaltag, tt=trial_type):
    ...
    if tt == "Animal":
        sequence_list = ["194882", "194883", "194884"]  # out of study , "189003"
        # Vini to write an if not previous statement
...
def exit_mode():
    ...
    """
    global Pi_RFID, Pi_exit, Pi_capture_1, Pi_end, chuck_lines
...
def acquire_weight(animaltag):
    ...
    for x in range(50):  # 50 lines*120ms per line=6s of data
        ...
        if relProb_float > 50:  # this could be soft coded....
...
class SaveData:
    ...
    def append_event(
        self, rotation, food_time, event_type, animaltag, wheel_position, FED_position
    ):
        """
...
def emergency_button(channel):
    """
    CAREFUL WITH FALSE POSITIVES WHEN OPENING ARDUINO
...
while True:
    ...
    while True:
        if MODE == 1:
            print("\nMODE 1\n")
            clean_tag = RFID_readtag("RFID1")  # might need 2s after reading to recover
        ...
        if MODE == 2 and GPIO.input(ard_pi_1):
            ...
            while True:
                ...
                if not same_animal:
                    ...
                    # Append data
                    save.append_event(
                        "+", "", "START", animaltag, wheel_position, FED_position
                    )
                    MODE = 3
                    choice_flag = False
                    break
        ...
        if MODE == 4 and GPIO.input(ard_pi_2) and not choice_flag:
            ...
            if event_list["Type"] == ["START"]:
                save.append_event(
                    "*", "", "BB2", animaltag, wheel_position, FED_position
                )
            ...
            if food_flag:
                ...
                save.append_event(
                    cycles_str, "", "Food_log", animaltag, wheel_position, FED_position
                )
            ...
            if run_flag:
                ...
                cycles_str = round(counter / cycle, 4)
                save.append_event(
                    cycles_str, "", "Run_log", animaltag, wheel_position, FED_position
                )
        ...
        if MODE == 0:
            ...
            if animal_left:
                ...
                if task_complete_flag:
                    sequence_index = (sequence_index + 1) % len(
                        sequence_list
                    )  # works for two animals
                    task_complete_flag = False
                break
        ...
        if (
            MODE == 4 and GPIO.input(ard_pi_5) and choice_flag
        ):  # log a maze event running wheel
            print("\nrunning wheel\n")
        ...
        if GPIO.input(Food_pod_retrieval) and pellet_complete_flag:
